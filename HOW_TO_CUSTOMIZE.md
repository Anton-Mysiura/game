...
PythonProject1/
│
├── config/                  ← ВСЕ ЩО РЕДАГУЄТЬСЯ ТУТ
│   ├── theme.py             кольори, розмір вікна, шрифти
│   ├── ui.py                кнопки, фони, панелі, текстури
│   ├── enemies.py           вороги + таблиці спавну
│   ├── loot.py              матеріали, предмети, кресленники
│   ├── heroes.py            герої та навички
│   ├── quests.py            квести і щоденні завдання
│   ├── shop.py              асортимент і ціни крамниці
│   ├── perks.py             перки і скіл-три
│   ├── locations.py         локації, події, карта світу
│   ├── validator.py         ⚠ перевірка помилок при старті
│   ├── hot_reload.py        🔄 Ctrl+F5 — без перезапуску
│   └── loader.py            ⛔ не редагуй — внутрішній
│
├── game/                    ← логіка гри (рідко потрібно)
├── scenes/                  ← екрани гри
│   ├── workshop.py          логіка майстерні
│   ├── workshop_ui.py       малювання майстерні
│   ├── battle_fighting.py   логіка бою
│   ├── battle_draw.py       малювання бою
│   └── ...
├── ui/                      ← UI компоненти
│   ├── components.py        Button, Panel, InventorySlot...
│   ├── assets.py            завантаження текстур/шрифтів
│   └── constants.py         → читає з config/theme.py
│
└── assets/
    ├── animations/          спрайти персонажів
    ├── textures/
    │   └── ui/
    │       ├── bg/          ← фони сцен (кидай PNG сюди)
    │       ├── button/      ← текстури кнопок
    │       ├── panel/       ← текстури панелей
    │       ├── icon/        ← UI іконки
    │       └── slot/        ← слоти інвентарю
    ├── fonts/               ← свої шрифти (.ttf)
    ├── sounds/
    └── music/
```
