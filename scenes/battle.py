...
class BattleScene(SceneWithBackground, SceneWithButtons):
    ...
    def draw(self, screen: pygame.Surface):
        ...
        # Панель бою
        self.battle_panel.draw(screen)

        # Спрайти персонажів
        player_sprite = assets.load_texture("characters", "player_attack", (128, 128))
        enemy_sprite = assets.load_texture("characters", self.enemy.sprite_name, (128, 128))
        ...
        self.player_hp_bar.draw(screen, self.player.hp, self.player.max_hp)

        # Ворог
        enemy_name = font.render(self.enemy.name, True, COLOR_TEXT)
        screen.blit(enemy_name, (SCREEN_WIDTH - 350, 390))
        self.enemy_hp_bar.draw(screen, self.enemy.hp, self.enemy.max_hp)

        # Лог подій
        self.log_box.draw(screen, COLOR_TEXT)
