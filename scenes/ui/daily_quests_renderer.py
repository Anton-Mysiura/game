...
class DailyQuestsRenderer(BaseRenderer):
    """
    Малює DailyQuestsScene.
    ...
    def draw(self, screen: pygame.Surface):
        ...
        for i, q in enumerate(quests):
            ...
            # Кнопка
            qid, btn = self.scene._claim_btns[i]
        ...
        # Feedback
        if self.scene._fb_timer > 0 and self.scene._feedback:
            fb_font = pygame.font.Font(None, 32)
            fb_col  = (100, 230, 100) if "🎉" in self.scene._feedback else (220, 160, 80)
            fb_surf = fb_font.render(self.scene._feedback, True, fb_col)
