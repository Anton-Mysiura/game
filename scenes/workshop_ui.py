"""
Малювання майстерні - весь UI workshop в одному місці.
Логіка знаходиться в workshop.py (WorkshopScene).

Цей файл містить тільки методи _draw_* і draw().
Не редагуй логіку тут - тільки зовнішній вигляд.
"""
import math
import pygame
...
# Імпортуємо всі layout-константи з workshop
from scenes.workshop import (
...
class WorkshopUIMixin:
    ...
    def _draw_ob_overlay(self, screen: pygame.Surface):
        ...
        step_d    = STEPS[min(self._ob_step, len(STEPS)-1)]
        highlight = step_d["hl"]
        ...
        if hl_rect:
            pulse = 0.6 + 0.4*abs(math.sin(self._ob_anim*3.0))
        ...
        bc  = tuple(int(c*(0.85+0.15*abs(math.sin(self._ob_anim*1.5))))
                    for c in (220,180,60))
        ...
        step_num = min(self._ob_step, len(STEPS)-1)
    ...
    def _draw_mode_tabs(self, screen):
        ...
        for i, btn in enumerate(self.mode_btns):
            btn.selected = (self.mode == modes[i])
            btn.draw(screen)
    ...
    def _draw_fc_type_switcher(self, screen, mp):
        ...
        for i, ob_i in enumerate(vis):
            ...
            sel   = (self._fc_blueprint is ob_i)
    ...
    def _draw_fc_slot(self, screen, slot_name, sx, mp):
        ...
        is_active = (self._fc_active == slot_name)
        sy        = FC_SLOTS_Y
    ...
    def _draw_fc_mat_list(self, screen, mp):
        ...
        # Заголовок
        slot_label = "Металеві матеріали" if self._fc_active == "blade" else "Усі матеріали"
        screen.blit(font_h.render(f"📦 {slot_label}", True, COLOR_GOLD),
                    (FC_MAT_X, FC_MAT_HDR_Y))
        screen.blit(font_sm.render("активний слот: " + ("⚔ Лезо" if self._fc_active == "blade" else "🪵 Рукоятка"),
                                    True, (120, 160, 120)),
        ...
        if total_count > FC_VISIBLE:
            ...
            thumb_y  = FC_MAT_Y + int((sb_h - thumb_h) * self._fc_mat_scroll / max(1, total_count - FC_VISIBLE))
    ...
    def _draw_craft_list(self, screen):
        ...
        for i, ob in enumerate(bps[self.scroll: self.scroll + VISIBLE]):
            bp  = ob.blueprint
            ...
            bg_clr = (80, 65, 20, 210) if i == self.sel else \
        ...
        screen.blit(assets.get_font(FONT_SIZE_SMALL).render(
            f"{self.scroll+1}–{min(self.scroll+VISIBLE, total)} з {total}", True, COLOR_TEXT_DIM),
    ...
    def _draw_queue_panel(self, screen):
        ...
        for slot_i in range(q.MAX_SLOTS):
            ...
            if slot_i < len(q.orders):
                ...
                if done:
                    # Пульсуюча анімація
                    alpha = int(180 + 75 * math.sin(self._anim_t * 4))
    ...
    def _draw_craft_info(self, screen):
        ...
        if self.sel == -1 or abs_idx >= len(bps):
    ...
    def _draw_bp_detail(self, screen, ob):
        ...
        # Кнопка дії
        self.action_btn.enabled = can and len(q.orders) < q.MAX_SLOTS and not ob.is_broken
        self.action_btn.draw(screen)
    ...
    def _draw_smelt_info(self, screen):
        ...
        self.action_btn.enabled = batches > 0
        self.action_btn.draw(screen)

    # ── Scrollbar ─────────────────────────────────────────────────

    def _draw_scrollbar(self, screen):
        if self.mode != "craft": return
    ...
    def _draw_msg(self, screen):
        if self.craft_msg_t <= 0: return
        alpha = min(255, int(self.craft_msg_t * 100))
        mut   = self._msg_mutation
        clr   = mut.color if mut else ((200, 80, 80) if self._msg_error else (100, 220, 100))
        font  = assets.get_font(FONT_SIZE_MEDIUM, bold=True)
        surf  = font.render(self.craft_msg, True, clr)
    ...
    def _draw_dismantle_list(self, screen):
        ...
        for i, item in enumerate(items[self.scroll: self.scroll + VISIBLE]):
            r   = pygame.Rect(LIST_X, LIST_Y + i * ROW_H, LIST_W, ROW_H - 4)
            mut = get_mutation(item)
            bg_clr = (80, 30, 30, 210) if i == self.sel else \
                     (55, 35, 30, 190) if r.collidepoint(mp) else \
                     (22, 18, 14, 160) if i % 2 == 0 else (28, 22, 18, 160)
            s = pygame.Surface((LIST_W, ROW_H - 4), pygame.SRCALPHA); s.fill(bg_clr)
            screen.blit(s, r.topleft)

            border_clr = mut.color if mut else ((180, 100, 60) if i == self.sel else (60, 50, 40))
            if i == self.sel:
    ...
    def _draw_dismantle_info(self, screen):
        ...
        items   = self._dismantle_items()
        abs_idx = self.scroll + self.sel
        if self.sel < 0 or abs_idx >= len(items):
            screen.blit(assets.get_font(FONT_SIZE_SMALL).render(
    ...
    def _draw_dismantle_queue_slots(self, screen, dq):
        ...
        for i, order in enumerate(dq.orders):
            ...
            if done:
                alpha = int(180 + 75 * _math.sin(self._anim_t * 4))
