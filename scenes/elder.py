...
class ElderScene(SceneWithBackground, SceneWithButtons):
    ...
    def _draw_hub_header(self, screen):
        ...
        if completable_count:
            pulse = 0.6 + 0.4 * math.sin(self._anim_t * 3)
    ...
    def _draw_quest_list(self, screen):
        ...
        for i, (status, quest) in enumerate(rows[self.scroll: self.scroll + VISIBLE]):
            r      = pygame.Rect(LIST_X, LIST_Y + i * ROW_H, LIST_W, ROW_H - 4)
            ...
            if status == "completable":
                pulse = 0.6 + 0.4 * math.sin(self._anim_t * 3 + i)
    ...
    def _draw_quest_info(self, screen):
        ...
        # Кнопка дії
        if status == "completable":
            self.btn_action.text    = "✦ Здати квест!"
            self.btn_action.enabled = True
        elif status == "available":
            self.btn_action.text    = "📜 Взяти квест"
            self.btn_action.enabled = True
        else:
            self.btn_action.enabled = False
            if status == "done":
                self.btn_action.text = "✓ Вже виконано"
            else:
                self.btn_action.text = "● В процесі"
    ...
    def _draw_dialog(self, screen):
        ...
        if page == len(pages) - 1:
            ...
            self.btn_next.text = "✓ Прийняти"
        else:
            self.btn_next.text = "Далі →"

        # Кнопки навігації
        self.btn_next.x = px + pw - 220
        self.btn_next.y = py + ph + 10
        self.btn_next.draw(screen)

        self.btn_prev.x = px + 20
        self.btn_prev.y = py + ph + 10
        self.btn_prev.enabled = (page > 0)
    ...
    def _draw_complete(self, screen):
        ...
        # Заголовок
        pulse = 0.8 + 0.2 * math.sin(self._anim_t * 2)
        ...
        # Кнопка "Закрити"
        self.btn_next.text    = "Закрити"
        self.btn_next.enabled = True
        self.btn_next.x       = px + pw // 2 - 100
        self.btn_next.y       = py + ph - 64
        self.btn_next.draw(screen)