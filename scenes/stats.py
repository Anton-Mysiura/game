...
class StatsScene(Scene):
    ...
    def draw(self, screen: pygame.Surface):
        ...
        # Вкладки
        for i, (btn, (tid, _)) in enumerate(zip(self.tab_btns, TABS)):
            btn.selected = (tid == self._tab)
            btn.draw(screen)
    ...
    def _draw_general(self, screen):
        ...
        self.hp_bar.draw(screen, p.hp, p.max_hp, "HP")
        self.xp_bar.draw(screen, p.xp, p.xp_next, "XP")
        py += 100
